...
- **Worker の CORS エラー** — FFmpeg ライブラリを `vendor/` フォルダから同一オリジン配信しています。**強制リロード**（Ctrl+Shift+R / Mac: Cmd+Shift+R）してから再試行してください
- **古いコードがキャッシュされている** — ブラウザのキャッシュをクリアするか、シークレットウィンドウで `http://localhost:8080` を開いてください
